...
vs0 = 30e3 + 15e3  # earth speed +  initial speed for a circular orbit, m/s (e.g. Pinoeer at 56 km/s, fastest)
stheta = -35 - .25 # angle of satellite compared to Jupiter (degrees)


initial_state = [0, 0, 0, ve0, -r0, r0*1.1, vs0*np.cos(np.radians(stheta)), vs0*np.sin(np.radians(stheta))]  # [x_jupiter, y_jupiter, vx_jupiter, vy_jupiter, x_sat, y_sat, vx_sat, vy_sat]
...
trail, = ax1.plot([], [], '-', color='yellow', lw=1, alpha=0.5)

ax2.set_xlim(-scale_lim*r0, scale_lim*r0)
ax2.set_ylim(0, np.max(np.sqrt(vx_sat_values**2 + vy_sat_values**2)) + 100000)
